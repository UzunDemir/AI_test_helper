...
st.subheader("AI ассистент по тестам (RAG)")
...
class KnowledgeBase:
    ...
    def split_text(self, text, max_tokens=400):
    ...
    def find_relevant(self, query, top_k=3):
        ...
        for idx in indices[0]:
            ...
            candidates.append(
                (chunk.text, chunk.doc_name, chunk.page_num)
            )
...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        if file.name not in st.session_state.kb.uploaded_files:

            with st.spinner("Processing document..."):

                st.session_state.kb.load_pdf(
                    file.getvalue(),
                    file.name
                )
...
if prompt := st.chat_input("Введите вопрос"):
    ...
    relevant = st.session_state.kb.find_relevant(prompt)
    ...
    if not relevant:
        ...
        context = "\n\n".join([
            ...
            for text, doc, page in relevant
        ])

        full_prompt = f"""
Answer strictly based on the educational materials.
...
"""
        ...
        if response.status_code == 200:
            ...
            for _, doc, page in relevant:
                sources += f"- {doc} page {page}\n"
