...
# ---------------- FILE UPLOAD ----------------
files=st.file_uploader("Загрузить PDF материалы", type="pdf", accept_multiple_files=True)
if files:
    for f in files:
        if f.name not in kb.files:
            ok=kb.load_pdf(f.read(),f.name)
...
if prompt:=st.chat_input("Введите вопрос"):
    ...
    if kb.index is None:
        st.warning("Сначала загрузите PDF материалы")
        st.stop()

    start=datetime.now()

    with st.spinner("Идёт поиск ответа... ⏳"):
        chunks=kb.retrieve(prompt)
...
"""
        ...
        try:
            ...
            answer="Ошибка получения ответа от модели"
    ...
    st.info(f"⏱️ Ответ найден за {(datetime.now()-start).total_seconds():.2f} сек")
